...
public class Stubs extends CalculationNode implements Loggable, Function {
	...
	final public Input<IntVectorParam<NonNegativeInt>> branchNrInput = new Input<>("branchNr", "node index of each stub", Input.Validate.OPTIONAL);
	final public Input<RealVectorParam<UnitInterval>> timeInput = new Input<>("time", "relative time of each stub along its branch", Input.Validate.OPTIONAL);
	final public Input<IntVectorParam<Int>> labelIndicatorInput = new Input<>("indicator", "label indicator (-1, 0, 1)", Input.Validate.OPTIONAL);
	// The number of stubs on each branch is estimated
	final public Input<IntVectorParam<NonNegativeInt>> stubsPerBranchInput = new Input<>("stubsPerBranch", "number of stubs per branch", Input.Validate.OPTIONAL);
	...
	final public Input<RealScalarParam<NonNegativeReal>> originInput = new Input<>("origin", "length of origin branch", Validate.OPTIONAL);
	...
	public void initAndValidate() {
		...
		IntVectorParam<Int> labelIndicator = labelIndicatorInput.get();
		if (labelIndicator != null) {
			labelIndicator.set(0);
...
//			}
		...
		// Number of stubs on each branch is estimated
			...
			IntVectorParam<NonNegativeInt> p = stubsPerBranchInput.get();
		...
		// Placement of stubs is estimated
			...
			branchNrInput.get().set(0);
			timeInput.get().set(0.5);
			
			
			// Do not initialise if we are resuming
			if (timeInput.get().size() > 1 & 
					branchNrInput.get().size() > 1 & 
					timeInput.get().size() == timeInput.get().size()) {
			...
			if (reset) {
				...
				Log.warning("Initialising stubs" + getNStubs() + timeInput.get().size() + " . " + branchNrInput.get().size());
	...
	public List<Stub> getStubs(){
		...
		for (int i = 1; i <= this.getStubDimension(); i ++) {
			int branchNr = this.getBranches().get(i);
			double time = getAbsoluteTimeOfStub(i);
			boolean include = this.includeStub(i);
			int labelIndicator = !hasIndicatorLabels() ? 0 : labelIndicatorInput.get().get(i);
			if (include) {
	...
	public int getNStubs() {
		...
		switch (this.stubMode) {
			...
			case branchCounts:
				...
				int nstubs = 0;
				for (int i = 0; i < this.stubsPerBranchInput.get().size(); i++) {
					nstubs += stubsPerBranchInput.get().get(i);
				}
				return nstubs;
				
			case exact:
				
				return branchNrInput.get().size() - 1; // Remove 1 for dummy index
	...
	public int getNStubsOnBranch(int nodeNr) {
		...
		switch (this.stubMode) {
			...
			case branchCounts:
				...
				if (nodeNr >= this.stubsPerBranchInput.get().size()) return 0;
				return this.stubsPerBranchInput.get().get(nodeNr);

			case exact:

				int nstubs = 0;
				for (int i = 0; i < branchNrInput.get().size(); i ++) {
					int b = branchNrInput.get().get(i);
	...
	public int getNStubsOnBranchWithIndicator(int nodeNr, int indicatorValue) {
		...
		for (int i = 0; i < branchNrInput.get().size(); i ++) {
			int b = branchNrInput.get().get(i);
			int ind = getLabelIndicators().get(i);
	...
	public IntVectorParam<NonNegativeInt> getBranches() {
		return branchNrInput.get();
	}

	public RealVectorParam<UnitInterval> getStubHeights() {
		return timeInput.get();
	}
	
	public IntVectorParam<Int> getLabelIndicators() {
	...
	public void storeDimensions() {
		...
		List<Double> x = getStubHeights().getElements();
		Double[] y = x.toArray(new Double[0]);
		
		
		storedTimes = getStubHeights().getElements().toArray(new Double[0]);
		storedBranches = getBranches().getElements().toArray(new Integer[0]);
		if (getLabelIndicators() != null) {
			storedLabelIndicators = getLabelIndicators().getElements().toArray(new Integer[0]);
		}
	...
	private void restoreDimensions() {
		...
		RealVectorParam<UnitInterval> times = this.getStubHeights();
		IntVectorParam<NonNegativeInt> branches = this.getBranches();
		IntVectorParam<Int> labelIndicators = this.getLabelIndicators();
		...
		for (int i = 0; i < oldDimension; i ++) {
			times.set(i, storedTimes[i]);
			branches.set(i, storedBranches[i]);
			if (labelIndicators != null) labelIndicators.set(i, storedLabelIndicators[i]);
		}
	...
	public List<Stub> getSortedStubsOnBranch(Node node){
		...
		for (int stubNr = 1; stubNr < this.getStubDimension(); stubNr++) {
			...
			int branchNr = branchNrInput.get().get(stubNr);
			
			if (branchNr == node.getNr()) {
				double time = getAbsoluteTimeOfStub(stubNr);
				int labelIndicator = !hasIndicatorLabels() ? 0 : labelIndicatorInput.get().get(stubNr);
	...
	public double getLabelIndicatorOfStub(int stubNr) {
		...
		return this.labelIndicatorInput.get().get(stubNr);
	}
	...
	public double getAbsoluteTimeOfStub(int stubNr) {
		...
		int branchNr = getBranches().get(stubNr);
		Node node = treeInput.get().getNode(branchNr);
		double relativeTime = timeInput.get().get(stubNr);
		double time;
		if (node.isRoot()) {
			time = node.getHeight() + originInput.get().get();
	...
	public int getStubDimension() {
		if (!getReversibleJump()) {
			return stubsPerBranchInput.get().size();
		} else {
			return branchNrInput.get().size();
		}
...
//	}
	...
	public List<Integer> getBranchesWithStubs() {
		...
		for (int stubNr = 0; stubNr < this.getStubDimension(); stubNr++) {
			if (!this.includeStub(stubNr)) continue;
			int branchNr = this.getBranches().get(stubNr);
...
//	}
	...
	public double getLogJacobian(double[] cachedBranchLengths) {
		...
		for (int stubNr = 0; stubNr < this.getStubDimension(); stubNr++) {
			...
			if (!this.includeStub(stubNr)) continue;
        	int stubBranchNr = this.getBranches().get(stubNr);
