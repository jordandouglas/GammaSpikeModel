...
public class StumpedTreePrior extends SpeciesTreeDistribution implements StubExpectation {
	...
	final public Input<RealScalarParam<? extends NonNegativeReal>> originInput = new Input<>("origin", "the time when the process started", Validate.OPTIONAL);
	...
	final public Input<RealScalarParam<? extends NonNegativeReal>> lambdaInput = new Input<>("lambda", "Birth rate (lambda)", Validate.REQUIRED);
	final public Input<RealScalarParam<? extends NonNegativeReal>> r0Input = new Input<>("r0", "reproduction number, or lambda / mu", Validate.REQUIRED);
	final public Input<RealScalarParam<? extends NonNegativeReal>> psiInput = new Input<>("psi", "Sampling rate (psi)", Validate.OPTIONAL);

	final public Input<RealScalarParam<? extends NonNegativeReal>> netDiversificationRateInput = new Input<>("netDiversificationRate", "net diversification rate, lambda - mu", Validate.XOR, lambdaInput);
	final public Input<RealScalarParam<? extends NonNegativeReal>> turnoverInput = new Input<>("turnover", "the ratio of death (mu) and birth (lambda) rates, mu / lambda", Validate.XOR, r0Input);
	final public Input<RealScalarParam<? extends UnitInterval>> samplingProportionInput = new Input<>("samplingProportion", "sampling rate psi divided by (psi + mu)", Validate.OPTIONAL);

//	final public Input<Function> r0Input = new Input<>("r0", "the ratio of birth (lambda) and death (mu) rates, lambda / mu");

	final public Input<RealScalarParam<? extends UnitInterval>> rhoInput = new Input<>("rho", "extant sampling probability (rho)", Validate.OPTIONAL);
...
//		return result;
	...
	private double getStubLogPForBranchWithSampling(int branchNr, double lambda, double mu, double psi, double rho) {
		...
		for (int stubNr = 0; stubNr < stubs.getStubDimension(); stubNr++) {
			...
			if (stubs.getBranches().get(stubNr) == branchNr) {
	...
	public double getLambda() {
		if (this.lambdaInput.get() == null) {
			double d = netDiversificationRateInput.get().get();
			double r = turnoverInput.get().get();
			return d / (1 - r);
...
//			}
			...
			return this.lambdaInput.get().get();
		}
	}
	...
	public double getMu() {
		if (this.r0Input.get() == null) {
			double lambda = this.getLambda();
			double t = turnoverInput.get().get();
			return t * lambda;
		} else {
			double lambda = this.getLambda();
			double r = r0Input.get().get();
			return lambda / r;
		}
//		double lambda = this.getLambda();
//		if (r0Input.get() != null) {
