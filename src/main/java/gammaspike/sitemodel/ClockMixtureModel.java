...
public class ClockMixtureModel extends SiteModel.Base {
	...
	final public Input<SimplexParam> clockWeightsInput = new Input<>("weights", "A vector of weights that each clock model receives", Input.Validate.REQUIRED);
	...
	final public Input<RealScalarParam<? extends PositiveReal>> shapeParameterInput = new Input<>("shape", "shape parameter of gamma distribution. Ignored if gammaCategoryCount 1 or less");
	...
	protected SimplexParam clockWeightsParameter;
	protected RealScalarParam<? extends PositiveReal> shapeParameter;
	...
	public void initAndValidate() {
		...
		if (this.nClocks != this.clockWeightsParameter.size()) {
		...
		if (muParameter == null) {
			muParameter = new RealScalarParam<PositiveReal>();
			muParameter.set(1.0);
	    }
...
//	}
	...
	public double getRateForCategory(int category, Node node) {
        ...
        return getCategoryRate(siteCategory) * getCategoryBranchRate(clockCategory, node) * muParameter.get();
	}
	...
	public double getProportionForCategory(int category, Node node) {
		...
		double pClock = this.clockWeightsParameter.get(clockCategory);
		return pSite*pClock;
