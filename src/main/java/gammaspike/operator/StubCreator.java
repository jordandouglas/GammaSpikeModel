...
public class StubCreator extends Operator {
	...
	final public Input<RealVectorParam<? extends PositiveReal>> totalTimeInput = new Input<>("totalTime", "totalTime", Input.Validate.OPTIONAL);
	...
	public double proposal() {
		...
		RealVectorParam<UnitInterval> times = stubs.getStubHeights();
		IntVectorParam<NonNegativeInt> branches = stubs.getBranches();
		IntVectorParam<Int> labelIndicators = stubs.getLabelIndicators();
		...
		if (create) {
			...
			for (int oldIndex = dimension-1; oldIndex >= indexToInsertAt; oldIndex--) {
				...
				times.set(newIndex, times.get(oldIndex));
				branches.set(newIndex, branches.get(oldIndex));
				if (labelIndicators != null) {
					labelIndicators.set(newIndex, labelIndicators.get(oldIndex));
				}
			}
			...
			// Set new values
			times.set(indexToInsertAt, u); // Relative time along branch
			branches.set(indexToInsertAt, nodeNr);
			if (labelIndicators != null) {
				labelIndicators.set(indexToInsertAt, indicatorNr);
			}
			...
			for (int i = 0; i < dimension; i ++) {
				...
				int b = tree != null ? stubs.getBranches().get(i) : nodeNr;
			...
			for (int i = 0; i < newDimension; i++) {
				...
				if (labelIndicators != null) {
					labelIndicators.set(i, labelIndicators.get(i));
				}
			}
