...
species.trees = getTrees("truth/truth-sampled-stubs.trees")
...
for (simnum in 1:NSIMS) {
  ...
  tryCatch({
    ...
    cat(sprintf("ERROR in simnum %d (rownum %d): %s\n", simnum, rownum, conditionMessage(e)))
    cat("Skipping this simulation...\n")
  })
}