...
process_replicate <- function(i, truth_data, burnin_frac) {
  ...
  regular_params <- intersect(params_to_analyze, colnames(post))
  nstubs_params_in_post <- grep("\\.nstubs$", colnames(post), value = TRUE)
  height_params_in_post <- grep("\\.height$", colnames(post), value = TRUE)
  treeLength_params_in_post <- grep("\\.treeLength$", colnames(post), value = TRUE)
  params_to_loop_over <- unique(c(regular_params, nstubs_params_in_post, height_params_in_post, treeLength_params_in_post))
  
  results_list <- lapply(params_to_loop_over, function(param_name) {

    truth_param_name <- if (endsWith(param_name, ".nstubs")) {
      "stubs.nstubs" # All "nstubs" params map to this single truth name
    } else if (endsWith(param_name, ".height")) {
      "tree.height" # All "height" params map to this single truth name
    } else if (endsWith(param_name, ".treeLength")) {
      "tree.treeLength" # All "treeLength" params map to this single truth name
    } else {
      param_name # Otherwise, the name is a 1-to-1 match
    }
    if (!(truth_param_name %in% params_to_analyze) || !(truth_param_name %in% colnames(truth_data))) {
      return(NULL)
    }
    ...
    true_val <- truth_data[[truth_param_name]][i]
...
params_to_check <- c("tree.height", "tree.treeLength", "lambda", "mu", "psi", "clockSD", "spikeShape")
failed_replicates_df <- all_results_df %>%
  filter((parameter %in% params_to_check | grepl("nstubs", parameter)) & in_hpd == FALSE)
...
if (nrow(failed_replicates_df) > 0) {
  ...
  message("All replicates successfully captured the true values within their 95% HPD intervals.")
}
